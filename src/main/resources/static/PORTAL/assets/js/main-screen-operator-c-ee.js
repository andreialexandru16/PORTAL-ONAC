var PAGE_NAME = "main-screen-operator-c-ee.js";
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.result == 'OK') {
                    ...
                    //check daca e cu putere< 10 si doar licenta de producere
                    else{

                        var url2 =  that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/checkPutereAndLicenta/'+ that.wsAndUserInfo.tert.id;
                            ...
                            success: function (data2) {
                                if (data2.result == 'OK') {
                                    ...
                                    if(data2.info=='true'){
                                        ...
                                        defer.reject("Nu aveti drepturi de raportare! (Sunteti inregistrat in sistem cu putere<10 si doar licenta de producere.)<br/>");
                                        Swal.fire({
                                            position: 'center',
                                            icon: 'error',
                                            html: "Nu aveti drepturi de raportare! (Sunteti inregistrat in sistem cu putere<10 si doar licenta de producere.)<br/>",
                                            focusConfirm: false,
                                            confirmButtonText: 'Ok',
                                            onClose: function(){
                                                window.top.location=$UTIL.WORDPRESS_URL+ '/';
                                            }
                                        });
                                    }
...
//user is "finished typing," do something
        function doneTyping () {
            ...
            var url =  '/dmsws/raportare/getTertRaportareListShort?&putere_min_10=true&searchStr='+ $text;
    ...
    loadPerioadaData: function () {
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/getPerioadaContabilaByZona',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
    ...
    loadFormulareData: function(){
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/getFormulareData',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
    ...
    renderFormulareData: function (data) {
        ...
        if(that.isAdmin){
            ...
            for(var i=0; i<data.contribEeList.length;i++){
                if(data.contribEeList[i].status.toLowerCase()=='in lucru'){
                    var item = $( "#sterge_declaratie_"+ data.contribEeList[i].id );
                    item.css("display","block");
                    item.removeAttr("hidden");
                }
            }
    ...
    determineVisibleOrInvisibleButtons: function(){
        ...
        try {
            lengthMaster = that.formulareData.contribEeList.length;
        } catch (e){}
    ...
    depuneDeclaratie: function(tip){
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/depuneDeclaratie',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
    ...
    editDeclaratie: function (id) {
        ...
        if (that.wsAndUserInfo!=null && that.wsAndUserInfo.tert.id!=null&&that.wsAndUserInfo.tert.id!=''){
        that.id_perioada_contabila= $("#select_perioada").val();
        var url=  './main-screen-operator-c-ee-detail.html?&id='+id;
        if (that.id_perioada_contabila!=null && that.id_perioada_contabila!=''){
    ...
    deleteDeclaratie: function(id){
        ...
        Swal.fire({
                ...
                return new Promise(function(resolve, reject) {
                    ...
                    $.ajax({
                        url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/deleteContrib',
                        type: 'post',
                        data: jReq,
                        headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                            'Accept': 'application/json',
