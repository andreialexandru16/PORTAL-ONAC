...
public class Ps4ECitizenConsultationPresenter extends PrepareModelFlowPresenter<Ps4ECitizenConsultationView> {
    ...
    public void prepareModel(String state) {
        ...
        if (hasAuthenticateUser()) {
            if(fromDepunere){
                ...
                else if (participatoryBudgetingService.checkIfHasRole(SecurityUtils.getToken(), "CONSULTARE_CETATENI").getInfo().equals("true")) {

                    getView().displayPoiConsultare();
                }
    ...
    private void loadBusinessPois() {
        ...
        if (participatoryBudgetingActionForm == ParticipatoryBudgetingActionForm.BUDGETING) {
            ...
            loadAllPois(token);
                 ...
                 && getView().getConsultationFormDraftValue()) {
            loadDraftPois(token);
        }
        else if ( participatoryBudgetingActionForm == ParticipatoryBudgetingActionForm.ADMIN
                && !getView().getConsultationFormDraftValue()) {
    ...
    public void loadDraftPois(String token) {
        List<ProjectInfo> infoPois = participatoryBudgetingService.getPOIs(token, Optional.empty(), Optional.empty(), Optional.of("NEPUBLICAT"), Optional.of(projectTypeCCId), Optional.empty()).getProjectInfoList();
        try {
    ...
    private void loadApprovedPois(String token) {
        List<ProjectInfo> infoPois = participatoryBudgetingService.getPOIs(token, Optional.empty(), Optional.empty(), Optional.of("PUBLICAT"), Optional.of(projectTypeCCId), Optional.empty()).getProjectInfoList();
        try {
            UI.getCurrent().getPage().executeJs("initBudgetMap($0, $1);", new ObjectMapper().writeValueAsString(infoPois), getView().getElement());
            if (participatoryBudgetingActionForm == ParticipatoryBudgetingActionForm.BUDGETING) {
                getView().setProjectsBudgetary(infoPois);
    ...
    public void loadAllPois(String token) {
        List<ProjectInfo> infoPois = participatoryBudgetingService.getPOIs(token, Optional.empty(), Optional.empty(), Optional.empty(), Optional.of(projectTypeCCId), Optional.empty()).getProjectInfoList();
        try {
            UI.getCurrent().getPage().executeJs("initBudgetMap($0, $1);", new ObjectMapper().writeValueAsString(infoPois), getView().getElement());
            if (participatoryBudgetingActionForm == ParticipatoryBudgetingActionForm.BUDGETING) {
                getView().setProjectsBudgetary(infoPois);
...
//            participatoryBudgetingService.
                ...
                UI.getCurrent().getPage().executeJs("swalInfo($0);", "Propunerea a fost publicata cu succes!");
            }
        }
