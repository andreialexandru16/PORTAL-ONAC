...
import ro.bithat.dms.smartform.gui.attribute.binder.CnpCuiAttributeValidator;
...
@SpringComponent
@UIScope
public class CnpCuiAttributeLinkComponent extends AttributeLinkGenericComponent {
    @Override
    public boolean canPostProcess(AttributeLink attributeLink) {
        return SmartFormSupport.getAttributeLinkDataType(attributeLink).equals(AttributeLinkDataType.CNP_CUI);
    }
...
//            }
            ...
            SmartFormSupport.bind(((Component)smartForm).getId().get(), new StringAttributeBinderBean(((Component) smartForm).getId().get(), attributeLink), component,
                    new CnpCuiAttributeValidator("",((Component)smartForm).getId().get()));
        }
    }
