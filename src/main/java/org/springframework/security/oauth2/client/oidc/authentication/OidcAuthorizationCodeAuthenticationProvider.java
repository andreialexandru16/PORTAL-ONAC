...
public class OidcAuthorizationCodeAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        // de aici incepe codul custom de a mapa userul cu userul din baza
        String userName = oidcUser.getName();
        String password = "openid"; // parola nu se foloseste dar clasa are nevoie de ceva.
        //aici login-ul a reusit --> vedem daca exista utilizatorul in baza de date si daca nu exista il inseram + datele din liferay
        try {
            dmswsUtilizatorService.insertUserWSO2(SecurityUtils.getToken(), userName);
        }catch(Exception e){
            logger.info("trying to insert wso2 user failed: " + userName);
        }
        logger.info("trying to lookup in database oidc user: " + userName);

        UserToken userToken = null;
        try {
            userToken = loginService.loginOidc(userName);
        } catch (Exception e) {
        ...
        final OidUsernamePasswordAuthenticationToken auth = new OidUsernamePasswordAuthenticationToken(principal, password, grantedAuths);
        auth.setOAuth2LoginAuthenticationToken(authenticationResult);
        return auth;
    }
