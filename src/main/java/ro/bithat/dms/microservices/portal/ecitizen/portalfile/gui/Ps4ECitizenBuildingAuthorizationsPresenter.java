...
public class Ps4ECitizenBuildingAuthorizationsPresenter extends PrepareModelFlowPresenter<Ps4ECitizenBuildingAuthorizationsView> {


    @Value("${poi.project.type.doc.urbanism.id}")
    private String projectTypeDocUrbanismId;

    @Value("${poi.project.aut.const.category.type.id}")
    private String projectCategoryAutConstId;
    ...
    public void prepareModel(String state) {
        ...
        getView().setPortalFileTable(budgetingService.getPOIs(SecurityUtils.getToken(), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty(),Optional.of(projectCategoryAutConstId)).getProjectInfoList());
    ...
    private void doSearch(String searchText) {
        ProjectInfoList portalFileList = budgetingService.getPOIsFiltered(SecurityUtils.getToken(), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty(),Optional.of(projectCategoryAutConstId),Optional.ofNullable(searchText));
    ...
    public void onAddProjectBtnClick(ClickEvent<NativeButton> clickEvent, TextField nr, TextField address, TextField lastName, TextField firstName, TextField type, AddProjectlDialogComponent dialogComponent){
        ...
        projectInfo.setIdCategoriePoi(Integer.valueOf(projectCategoryAutConstId));
...
//            participatoryBudgetingService.
            ...
            UI.getCurrent().getPage().executeJs("swalInfo($0);", "Autorizatia a fost incarcata cu succes!");
