...
var PageManager = {
    ...
    COD_LOV_GRAD: "LOV_LP_GRAD_EE",
    COD_LOV_STARE: "LOV_LP_STARE_EE3",
    exportXlsx: function () {
        $UTIL.waitForLoading();
    ...
    getInfoFilters: function(){
        ...
        var judet=$("#judet").val();
        var nr_legitimatie=$("#nr_legitimatie").val();
        var data_emitere=$("#data_emitere").val();
        var data_expirare=$("#data_expirare").val();
        var stare_tiparire=$("#stare_tiparire").val();
        var legitimatie={
            nume_complet:nume_complet,
            grad:grad,
            localitate:localitate,
            judet:judet,
            nr_legitimatie:nr_legitimatie,
            data_emitere:data_emitere,
            data_expirare:data_expirare,
            stare_tiparire:stare_tiparire
        }
            ...
            success: function (data) {
                if (data.result == 'OK') {
                    ...
                    if (data.lov != null && data.lov.length != 0) {
                        var lov = data.lov;
                        $('#stare_tiparire').append(that.getOptionListItemHtml('0', 'Selecteaza stare'));
                        for(i=0;i<lov.length;i++){
                            $('#stare_tiparire').append(that.getOptionListItemHtml(lov[i].id, lov[i].valoare));
                        }
                    }
                }
    ...
    initLovGrad: function() {
        var PROC_NAME = "PageManager.initLovGrad";
        $UTIL.log(PAGE_NAME, PROC_NAME, 'Enter', 0);
        debugger;
        var that = this;
        var defer = $.Deferred();
        var url = "/dmsws/lov/values_by_code/" + PageManager.COD_LOV_GRAD;
        $.ajax({
            url: url,
            success: function (data) {
                if (data.result == 'OK') {
                    ...
                    if (data.lov != null && data.lov.length != 0) {
                        var lov = data.lov;
                        $('#grad').append(that.getOptionListItemHtml('0', 'Selecteaza grad'));
                        for(i=0;i<lov.length;i++){
    ...
    getListaInregistrari: function (jReq) {
        ...
        var url ="/dmsws/anre/getAutorizatiiElectricieni";
    ...
    buildPagination: function () {
        ...
        var url ="/dmsws/anre/getAutorizatiiElectricieniCount/";
