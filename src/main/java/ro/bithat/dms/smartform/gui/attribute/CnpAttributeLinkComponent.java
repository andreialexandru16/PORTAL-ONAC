...
import ro.bithat.dms.smartform.gui.attribute.binder.CnpAttributeValidator;
...
@SpringComponent
@UIScope
public class CnpAttributeLinkComponent extends AttributeLinkGenericComponent {
    @Override
    public boolean canPostProcess(AttributeLink attributeLink) {
        return SmartFormSupport.getAttributeLinkDataType(attributeLink).equals(AttributeLinkDataType.CNP);
    }
...
//            }
            ...
            SmartFormSupport.bind(((Component)smartForm).getId().get(), new StringAttributeBinderBean(((Component) smartForm).getId().get(), attributeLink), component,
                    new CnpAttributeValidator("",((Component)smartForm).getId().get()));
        }
    }
