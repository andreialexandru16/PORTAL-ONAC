        ...
        if (params.has('orderId')){
            ...
            if(that.orderId) {
                $.ajax({
                    ...
                    url:  $UTIL.PORTAL_URL+'/dmsws/utilizator/getStatusOrderApi',
                    success: function (data) {



                        if (data.actionCode === 0 || data.actionCode == "0") {
                            location.replace("/contul-meu-cererile-mele");
                        } else {
                            Swal.fire({
                                position: 'top',
                                icon: "info",
                                html: data.actionCodeDescription,
                                focusConfirm: false,
                        ...
                        Swal.fire({
                            position: 'top',
                            icon: "info",
                            html: "Plata nu a putut fi verificata",
                            focusConfirm: false,
                            confirmButtonText: "Ok",
                            onClose: () => {
                            window.top.location.href = $UTIL.WORDPRESS_URL ;
