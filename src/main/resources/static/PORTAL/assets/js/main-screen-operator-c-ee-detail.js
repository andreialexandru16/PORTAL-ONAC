var PAGE_NAME = "main-screen-operator-c-ee-detail.js";
...
var FileManager = {
    ...
    compileAllTemplates: function () {
        this.templates['tmpl_formulare_activitati_list'] = $('#tmpl_formulare_activitati_list').html();
    ...
    mandatoryFunctions: function () {
        ...
        that.getWsAndUserInfo().then(
            function(){
                ...
                if (params.has('id')){
                    that.id_contrib_ee = params.get('id');
                }
    ...
    loadFormulareDetailData: function(){
        ...
        var req = {
            "id_contrib_ee": that.id_contrib_ee
        };
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/getFormulareDetailData',
            type: 'post',
            data: jReq,
            ...
            success: function (data) {
                if (data.status == 'OK') {
                    ...
                    if(that.formulareDetailData!=null && that.formulareDetailData.contribEeList.length!=0){
                        currentObj=that.formulareDetailData.contribEeList[0];
                    }
                    for(var i=0;i<data.contribEeList.length;i++){
                        for(var j=0;j<data.contribEeList[i].detailList.length;j++){

                            var current= data.contribEeList[i].detailList[j];
                            if(typeof current !='undefined' && typeof current.valoare_minima !='undefined'){
                                data.contribEeList[i].detailList[j].valoare_minima_view=current.valoare_minima.toLocaleString();

                            }
                            if(typeof current !='undefined' && typeof current.valoare_declarata !='undefined' && current.valoare_declarata!=null){
                                data.contribEeList[i].detailList[j].valoare_declarata_view=current.valoare_declarata.toLocaleString();

                            }
                        }

                        if(data.contribEeList[i].activitatiList!=null){
                            for(var j=0;j<data.contribEeList[i].activitatiList.length;j++){

                                var current= data.contribEeList[i].activitatiList[j];
                                if(typeof current !='undefined' && typeof current.valoare !='undefined' && current.valoare!=null){
                                    data.contribEeList[i].activitatiList[j].valoare_view=current.valoare.toLocaleString();
                    ...
                    that.renderFormulareActivitatiData(data);
    ...
    renderFormulareActivitatiData: function (data) {
    var that = this;


    $("#container_activitati").empty();
    var html = that.renderTemplateNonAsync(that, "tmpl_formulare_activitati_list", data);
    $("#container_activitati").html(html);
    $(".input_valoare_activitate").number(true, 2, '.', ' ');
},
    ...
    renderFormulareDetailData: function (data) {
        ...
        $(".input_valoare_declarata").number(true, 2, '.', ' ');
    ...
    determineVisibleOrInvisibleButtons: function(){
        ...
        try {
            statusMaster = that.formulareDetailData.contribEeList[0].status;
        } catch (e){}
        ...
        if (typeof statusMaster !== 'undefined' && statusMaster === 'Depusa'){
            ...
            $("#buton_depune_declaratie_finala").css('display', 'inline-block');
        }
    },

    updateValoareCalculata: function(id,id_contrib_ee,coeficient,valoare_minima,element_id,calculated_element_id){
        var that = this;
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/updateDeclaratieDetailValoareCalculata',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
            ...
            success: function (data) {
                if (data.status == 'OK') {
                    that.updateValoareCalculataRand(id,id_contrib_ee,coeficient,valoare_minima,element_id,calculated_element_id,data.valoareCalculata);
                    defer.resolve();
                } else {
                    Swal.fire({
                        position: 'top',
    ...
    updateAlteActivitati: function(id, id_contrib_ee, input_curent){
        var that = this;


        var valoare = $("#"+input_curent).val();
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/updateAlteActivitati',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
            ...
            success: function (data) {
                if (data.status == 'OK') {
                    that.animateElementOk("#"+input_curent);
                                  defer.resolve();
                } else {
                    Swal.fire({
                        position: 'top',
                    ...
                    that.animateElementErr("#"+input_curent);
    ...
    updateReprezentant: function(element_id){
        ...
        if($UTIL.validateEmail(mail_contact)){
            ...
            var defer = $.Deferred();
            $.ajax({
                url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/updateReprezentant',
                type: 'post',
                data: jReq,
                headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                    'Accept': 'application/json',
    ...
    updatePersoanaContact: function(element_id){
        ...
        if($UTIL.validateEmail(mail_contact)){
            ...
            var defer = $.Deferred();
            $.ajax({
                url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/updatePersoanaContact',
                type: 'post',
                data: jReq,
                headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                    'Accept': 'application/json',
    ...
    updateMentiuni: function(element_id){
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/updateMentiuni',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
    ...
    updateAlteActivitatiObs: function(id, id_contrib_ee, input_curent){
        var that = this;


        var observatii = $("#"+input_curent).val();
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/updateAlteActivitatiObs',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
    ...
    descarcaDeclaratie: function(){
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/descarcaDeclaratie',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
    ...
    depuneDeclaratieFinala: function(){
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/salveazaSiDepuneDeclaratie',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
    ...
    deleteAttachedFile: function(id_fisier, id_contrib_ee_fisiere){
        var that = this;

        var req = {
            "id_fisier": id_fisier,
            "id_contrib_ee_fisiere" : id_contrib_ee_fisiere
        };
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/deleteAttachedFile',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
    ...
    saveAttachedFileName: function(id_fisier, id_contrib_ee_fisiere){
        var that = this;

        var newName = $("#file_name_" + id_contrib_ee_fisiere).val().trim();
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/saveAttachedFileName',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
            ...
            success: function (data) {
                if (data.status == 'OK') {
                    that.loadDocumenteContributiiByZona().then(function(){
                        that.animateElementOk('#file_name_' + id_contrib_ee_fisiere);
                    });
                    defer.resolve();
                } else {
                    that.loadDocumenteContributiiByZona().then(function(){
                        that.animateElementErr('#file_name_' + id_contrib_ee_fisiere);
                    });
                    defer.reject("A aparut o eroare");
                }
            },
    ...
    continueUploadFile: function(id_tip_doc){
        ...
        if (typeof upload_file !== 'undefined' && upload_file !== null && upload_file !== ''){
            ...
            var url = that.wsAndUserInfo.uploadWsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/uploadDocumentContributii';
            ...
            formData.append('id_contrib_ee', that.id_contrib_ee);
            formData.append('zona', "CONTRIB_EE");
    ...
    loadDocumenteContributiiByZona: function(){
        ...
        var defer = $.Deferred();
        $.ajax({
            url: that.wsAndUserInfo.wsUrl + '/anre_contributii_ee/' + that.wsAndUserInfo.userToken.token + '/getDocumenteContributiiByZona',
            type: 'post',
            data: jReq,
            headers: ObjTools.mergeObjects(AjaxTools.corsHeaders, {
                'Accept': 'application/json',
    ...
    updateValoareCalculataRand: function(id,id_contrib_ee,coeficient,valoare_minima,element_id,calculated_element_id,valoare_calculata){
        var that = this;
    ...
    backToMaster: function(){
        var that=this;
        var url=  './main-screen-operator-c-ee.html';
